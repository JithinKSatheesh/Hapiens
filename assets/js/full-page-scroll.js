...
(function () {
	...
	fullScroll.prototype.buildPublicFunctions = function () {
		...
		this.mouseWheelAndKey = function (event) {
			if (event.deltaY > 0.8 || event.keyCode == 40) {	
				_self.defaults.currentPosition ++;
				_self.changeCurrentPosition(_self.defaults.currentPosition);				
			} else if (event.deltaY < - 0.8|| event.keyCode == 38) {
				_self.defaults.currentPosition --;
				_self.changeCurrentPosition(_self.defaults.currentPosition);	
			}
			// testing scroll sensitivity
		...
		this.animateScroll = function () {
			...
			for (var i = 0; i < this.ul.childNodes.length; i++) {
					...
					if(this.defaults.currentPosition == 0)
					{
						document.body.style.backgroundImage = "url('assets/image/Slide1BG.png')";
						$('.nav_bar').removeClass('nav_list_active');
						$('.dots').removeClass('nav_list_active');
					}
					else if (this.defaults.currentPosition == 1)
					{
						document.body.style.backgroundImage = "url('assets/image/Slide2BG.png')";
						$('.nav_bar').removeClass('nav_list_active');
						$('.dots').removeClass('nav_list_active');
					}
					else if (this.defaults.currentPosition == 2)
					{
						document.body.style.backgroundImage = "url('assets/image/Slide3BG.png')";
						$('.nav_bar').removeClass('nav_list_active');
						$('.dots').removeClass('nav_list_active');
					}
					else
					{
						$('.nav_bar').addClass('nav_list_active');
